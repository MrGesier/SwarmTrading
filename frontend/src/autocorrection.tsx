...
export function Autocorrection(){
  ...
  return <section className="autocorrection-panel" aria-label="Autocorrection du code">
    <header><div><small>EXPÉRIENCE LOCALE · CODE UNIQUEMENT</small><h3>Autocorrection / Code Engineer</h3><p>Incident mesuré → proposition de code isolée → tests et comparaison paper → revue humaine.</p></div><span className={state?.worker_online?'positive':'negative'}>{state?.worker_online?'Worker local connecté':'Worker arrêté'}</span></header>
    <details open><summary>Ce qui évolue automatiquement aujourd’hui</summary>
      ...
      <p><b>Code :</b> les incidents persistants déclenchent au plus une proposition Codex par 24 h sur une politique de signal isolée. Les contrats, régressions et données Hyperliquid réservées à la validation sont testés avant revue. La démonstration manuelle de présentation reste facultative.</p>
    ...
    <details><summary>Agents de recherche · OpenRouter gratuit</summary>
      <p>{state?.openrouter?.configured?'Clé configurée · connexion à tester':'Fournisseur non connecté · OPENROUTER_API_KEY absent'}. Budget partagé : {state?.openrouter?.budget?.remaining??'—'} / {state?.openrouter?.budget?.limit??'—'} tentatives restantes sur 24 h glissantes ; réserve incidents : {state?.openrouter?.budget?.reserve??'—'}.</p>
      <p>Quota OpenRouter observé : {state?.openrouter?.remote_budget?.remaining??"indisponible"} / {state?.openrouter?.remote_budget?.limit??"—"} ; jour UTC, distinct du budget local.</p>
