...
type DarwinState = {
  shared_portfolio?: SharedPortfolioState;
    ...
    <div className="darwin-lab">
      <div className="darwin-hero">
        <div>
          ...
          <p>Le portefeuille commun engage les allocations paper. Le laboratoire compare séparément les stratégies ; ses résultats ne sont pas le capital du portefeuille.</p>
        </div>
      ...
      <details className="advanced-panel"><summary>Architecture technique des agents</summary>{agents && <section className="agent-architecture">
        ...
        <div className="agent-safety-note"><ShieldCheck size={15} /> Rôles configurés : leur présence ne prouve pas un appel LLM. Les appels et replis effectivement enregistrés sont visibles dans Factory. FORGE, CERBERUS et HERMES restent du code déterministe.</div>
      </section>}</details>

      {state.shared_portfolio && <SharedPortfolioPanel p={state.shared_portfolio}/>}
      <section className="darwin-card"><h3>Que fait la recherche ?</h3><p>Les essais du laboratoire ne sont pas des positions financées par ton portefeuille. Les appels LLM réels, leurs réponses et les mutations sont suivis dans Factory.</p><a href="/factory">Voir les appels et les expériences →</a></section>
      <details className="advanced-panel"><summary>Laboratoire · stratégies, essais et mémoire · hors capital commun</summary>
      <div className="darwin-kpis">
        <div><small>STRATÉGIES EN LABORATOIRE</small><b>{state.population}</b><span>{state.status_counts.CHALLENGER ?? 0} challengers · {state.historical_population} ever created</span></div>
        ...
        <div><small>RÉFÉRENCE DU LABORATOIRE / STRATÉGIE</small><b>${fmt(state.paper.notional_usd, 0)}</b><span>{fmt(state.paper.fee_bps, 1)} bp / fill</span></div>
      ...
      {capital && <details className="advanced-panel"><summary>Comptes indépendants du laboratoire · hors portefeuille commun</summary><section className="darwin-card" aria-label="Suivi du capital paper">
      ...
      <section className="darwin-card trade-journal">
        <div className="darwin-card-head"><span>Derniers trades des comptes de recherche · USD</span><small>{state.symbol} · {state.mode} · toutes les stratégies</small></div>
