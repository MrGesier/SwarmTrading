...
import {MultiScale,aggregateCandles} from "./multiscale";
...
function App() {
  ...
  useEffect(()=>setContext("auto"),[horizon]);

  const replay = route === "/replay";

  const original = replay ? (frames[cursor] ?? live) : live;

  const selected=original?.horizons?.[String(horizon)];

  const s:State|null=original?{...original,

    candles:aggregateCandles(original.candles,chartInterval),

    ...(selected?{swarm:{...original.swarm,...selected.swarm},entropy:selected.entropy,intent:selected.intent,timeline:selected.timeline,triggers:selected.triggers,

    history:original.history.map(row=>({...row,market_entropy:row.horizon_metrics?.[String(horizon)]?.market_entropy??row.market_entropy,

      swarm_entropy:row.horizon_metrics?.[String(horizon)]?.swarm_entropy??row.swarm_entropy,consensus:row.horizon_consensus?.[String(horizon)]??0}))}:{}),
  ...
  useEffect(() => {
    ...
    function connect() {
      ...
      ws = new WebSocket(
        ...
        `ws://127.0.0.1:8000/ws?symbol=${symbol}&mode=${mode}`,
      ...
      <aside className="sidebar">
        ...
        <div className="engine-status">
          ...
          <div>
            ...
            <small>{original?.swarm.raw ?? 320} strategy genomes</small>
        ...
        <div className="read-only">
          ...
          <ShieldCheck size={15} /> READ ONLY <span>LOCAL</span>
        ...
        <main>
          ...
          <div className="market-bar">
            ...
            <div className="symbol-pick">
              ...
              <div>
                ...
                <small>
                  ...
                  {mode === "live" ? "Binance spot" : "Synthetic market"}
            ...
            <div className="stream-controls">
              ...
              <select aria-label="Horizon de contexte" value={context} onChange={e=>setContext(e.target.value)}><option value="auto">Contexte auto</option>{[1,5,30,60,180].filter(h=>h>horizon).map(h=><option key={h} value={h}>Contexte {horizonLabel(h)}</option>)}</select>
            ...
            <div className="empty">
              ...
              <p>
                ...
                {mode === "live"
                  ...
                  ? "Connecting to Binance and reconciling the order book."

                  : "Starting the local engine and warming up the strategy population."}

              </p>

              <p>The backend must be running on port 8000.</p>
            ...
            <>
              ...
              <div className="metrics">
                ...
                <Metric
                  ...
                  sub={`${signed(s.intent.score, 0)} / 100 · local, non calibré`}
                ...
                <Metric
                  ...
                  sub={`${s.swarm.active} active / ${s.swarm.raw} total · selected horizon`}
              ...
              {original?.horizons&&<MultiScale state={original} horizon={horizon} context={context}/>}
                      ...
                      <Panel
                        ...
                        label={
                          ...
                          <span className="chart-tools">
                            ...
                            <select aria-label="Durée des bougies" value={chartInterval} onChange={e=>setChartInterval(Number(e.target.value))}>{[1,5,30,60,180].map(t=><option key={t} value={t}>Bougies {horizonLabel(t)}</option>)}</select>
