...
Open http://127.0.0.1:8000/factory. Select Simulation for offline synthetic market data.
...
- `python -m pytest backend -q`: 58 passed (two upstream TestClient deprecation warnings).
...
Unobserved market gaps are not reconstructed. A crash can lose ticks since the last checkpoint.
...
- Atomic epoch/mutation/checkpoint transaction and crash fault-injection coverage; gap-aware evidence duration.
- Fixed G0 shadow cohort, identical fee policies enforced across resumed config changes,
